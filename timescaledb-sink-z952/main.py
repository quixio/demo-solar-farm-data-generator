...
class TimescaleDBSink(BatchingSink):
    ...
    def _create_table_if_not_exists(self):
        ...
        with self._connection.cursor() as cursor:
            ...
            try:
                ...
                # Ignore if hypertable creation fails (table might already exist)
                pass
    ...
    def write(self, batch: SinkBatch):
        ...
        insert_query = f"""
        INSERT INTO {self._table_name} (timestamp, {self._json_column})
        VALUES (%s, %s)
        """
        ...
        with self._connection.cursor() as cursor:
            for item in batch:
                ...
                # item.value is already a parsed dictionary, no need for json.loads
                cursor.execute(insert_query, (ts, json.dumps(item.value)))
            self._connection.commit()
...
input_topic = app.topic(os.environ.get("input", "input_topic"), key_deserializer="string")
