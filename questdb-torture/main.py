...
class QuestDBSink(BatchingSink):
    ...
    def setup(self):
        ...
        try:
            ...
            # Test connection by creating sender
    ...
    def _parse_solar_data(self, item):
        ...
        According to the schema analysis, the message structure is:
        - item.value contains the JSON-encoded solar panel data as a string
        """
        try:
            # Debug print to show raw message structure
            print(f'Raw message: {item.value}')
            
            # The value field contains the JSON string with solar data
            if isinstance(item.value, str):
                solar_data = json.loads(item.value)
            elif isinstance(item.value, dict) and 'value' in item.value:
                # If the message is already parsed and has nested structure
    ...
    def write(self, batch: SinkBatch):
        ...
        """
        if not self._sender:
            raise Exception("QuestDB sender not initialized. Call setup() first.")
        ...
        while attempts_remaining:
            try:
                ...
                print(f"Successfully wrote batch of {len(list(batch))} messages to QuestDB table '{self._table_name}'")
